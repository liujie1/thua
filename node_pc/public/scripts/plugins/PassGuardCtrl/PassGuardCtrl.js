...
//Windows IE32
var PGEdit_IE32_CLASSID = "D84B58E2-329D-4506-A300-3128B521F5EE";
var PGEdit_IE32_CAB = "";
var PGEdit_IE32_EXE = "THFPassGuardX.exe";
var PGEdit_IE32_VERSION = "1,0,0,1";
//Windows IE64
var PGEdit_IE64_CLASSID = "D84B58E2-329D-4506-A300-3128B521F5EE";
var PGEdit_IE64_CAB = "";
var PGEdit_IE64_EXE = "THFPassGuardX64.exe";
var PGEditt_IE64_VERSION = "1.0.0.1";
//Windows 非IEcheck
var PGEdit_FF = "THFPassGuardFF.exe";
var PGEdit_FF_VERSION = "1.0.0.1";
//Win10Edge / Chrome42+
var PGEdit_Edge = "THFPassGuardEdge.exe";
var PGEdit_Edge_Mac = "THFPassGuardEdge.pkg";
var PGEdit_Edge_VERSION = "1.0.0.1";
//Mac OS
var PGEdit_MacOs = "THFPassGuard.pkg";
	...
	jQuery.extend(jQuery.pge, {
		...
		prototype: {
				...
				if (!isInstalled) {
					...
					try {
						...
						if (this.osBrowser == 1 || this.osBrowser == 3) {//IE
							control.input7 = my;//设置公钥
							control.input10 = 1;
							code = control.output104;//加密接口
						} else if (this.osBrowser == 2 || this.osBrowser == 4 || this.osBrowser == 5) {//非IE
							control.input(200, my);//设置公钥
			...
			getVersion: function (callf) {
				try {
					if (this.osBrowser == 1 || this.osBrowser == 3) {
						var comActiveX = new ActiveXObject("THFPassGuard.PassGuard.1");
						//alert(comActiveX.output35);
						return comActiveX.output35;
					} else if (this.osBrowser == 2 || this.osBrowser == 6 || this.osBrowser == 8) {
						var arr = new Array();
						if (this.osBrowser == 6) {
							var pge_info = navigator.plugins['THFPassGuard'].description;
						} else if (this.osBrowser == 8) {
							var pge_info = navigator.plugins['THFPassGuard'].description;
						} else {
							var pge_info = navigator.plugins['THFPassGuard32'].description;
						}
			...
			getpgeHtml: function () {
				if (this.osBrowser == 1 || this.osBrowser == 3) {
					return '<span id="' + this.settings.pgeId + '_pge" class="' + this.settings.passLoginStyle + '"><OBJECT ID="' + this.settings.pgeId + '" CLASSID="CLSID:' + this.pgeditIEClassid + '" codebase="'
						...
						+ '<param name="input2" value="' + this.settings.pgeEreg1 + '"><param name="input12" value="' + this.settings.pgePlaceHolder + '"><param name="FontName" value="' + this.settings.placeholderFont + '"><param name="FontSize" value="' + this.settings.placeholderSize + '"><param name="input3" value="' + this.settings.pgeEreg2 + '"></OBJECT></span>'
					...
					var pgeOcx = '<embed onmouseover="' + ff + '" onfocus="' + this.settings.pgeOnfocus + '" onblur="' + this.settings.pgeOnblur + '" ID="' + this.settings.pgeId + '"  maxlength="' + this.settings.pgeMaxlength + '" input_600="'+this.settings.pgePlaceHolder+'"   input_601="'+this.settings.pgeBackColor+'"  input_602="'+this.settings.placeholderFontColor+'" input_603="' + this.settings.placeholderFont + '"   input_604="' + this.settings.placeholderSize + '"  input_2="' + this.settings.pgeEreg1 + '" input_3="' + this.settings.pgeEreg2 + '" edittype="' + this.settings.pgeEdittype + '" type="application/thfpass-guard-x32" tabindex="' + this.settings.pgeTabindex + '" class="' + this.settings.pgeClass + '" ';
					...
					this.checkInstall(1, function (obj) {
						...
						if ((obj.getConvertVersion(objVersion) < obj.getConvertVersion(PGEdit_Edge_VERSION)) && PGEdit_Update == "1") {
							...
							if (obj.osBrowser == 10) {
								...
								strr = "this.type=\'password\';", typeStr = "text";
								if(!pgeCtrl.env.ie){
								strr2 = "this.type=\'text\';";
							}
							}
							if(pgeCtrl.env.ie){
								strr = "", typeStr = "password";
							}
							...
							if (isSupportPlaceholder()) {
								var pgePwdEl = $('<input placeholder="' + obj.settings.pgePlaceHolder + '" type="'+ typeStr +'" onfocus="' + strr + 'pgeCtrl.openProt(\'' + winId + '\',this.id);pgeCtrl.setCX(this);' + obj.settings.pgeOnfocus + '" onkeydown="pgeCtrl.setSX(event,\'' + obj.settings.pgeOnkeydown + '\',this);" onclick = "pgeCtrl.setCX(this)" onblur = "'+strr2+'pgeCtrl.closeProt(\'' + winId + '\',this.id);' + obj.settings.pgeOnblur + '" id = "' + id + '" style="ime-mode:disabled" tabindex="2" class="' + obj.settings.pgeClass + '"/>');
								$("#" + id + "_down").parent().html(pgePwdEl);
							} else {
								var pgePwdEl = $('<input placeholder="' + obj.settings.pgePlaceHolder + '" type="'+ typeStr +'" onfocus="' + strr + 'pgeCtrl.openProt(\'' + winId + '\',this.id);pgeCtrl.setCX(this);' + obj.settings.pgeOnfocus + '" onkeydown="pgeCtrl.setSX(event,\'' + obj.settings.pgeOnkeydown + '\',this);" onclick = "pgeCtrl.setCX(this)" onblur = "'+strr2+'pgeCtrl.closeProt(\'' + winId + '\',this.id);' + obj.settings.pgeOnblur + '" id = "' + id + '" style="ime-mode:disabled" tabindex="2" class="' + obj.settings.pgeClass + '"/>');
								var hidePgePwdEl = $('<input placeholder="' + obj.settings.pgePlaceHolder + '" type="text" value="' + obj.settings.pgePlaceHolder + '" class="' + obj.settings.pgeClass + '" id="z' + id + '" />');
								// console.log('<input placeholder="' + obj.settings.pgePlaceHolder + '" type="text" value="' + obj.settings.pgePlaceHolder + '" class="' + obj.settings.pgeClass + ' />');
								hidePgePwdEl.on("focus", function () {
									$(this).hide();
									pgePwdEl.show().click().focus();
								});
								pgePwdEl.on("blur", function () {
									var hasval = pgeCtrl.pwdGetEById(obj.settings.pgeId).value.length > 0;
									if (!hasval) {
										$(this).hide();
										hidePgePwdEl.show();
									}
								...
								$("#" + id + "_down").parent().html(pgePwdEl);
								pgePwdEl.parent().append(hidePgePwdEl);
								pgePwdEl.hide();
							}
			...
			checkInstall: function (s, callf) {
				try {
					if (this.osBrowser == 1 || this.osBrowser == 3) {
						...
						/*	jQuery.ajax( {timeout : 300,url : urls,dataType : "jsonp",
						...
						jsonp({
							...
							success: function (x) {
								...
								if (logFlag) console.log("check is install:" + true);
								objVersion = x.data;
								//alert(11);
								if (!!callf) callf(obj);
							},
			...
			getEnStr: function (sKey, jsonStr) {
				...
				for (var i = 0; i < sKey.length; i++) {
					lx = String.fromCharCode(sKey.charAt(i).charCodeAt(0) ^ neiKey[i]);
					fkey += lx;
				}
			...
			pgInitialize: function () {
				if (isInstalled) {
					...
					try {
						var color = this.settings.pgeBackColor;
						var pgeId = this.settings.pgeId;
						var i = parseInt(color.replace("#", ""), 16);
						setTimeout(function () {
							var pgd = document.getElementById(pgeId);
							if (pgd && !isNaN(i)) {
								pgd.BackColor = i;
								pgd.ForeColor = 7763574;
							}
						}, 60);
					} catch (ex) {
			...
			instControl: function (id, obj) {
				...
				RZCIJSON = { "rankey": this.settings.pgeRZRandNum, "datab": this.settings.pgeRZDataB, "datac": datac };
				/*	jQuery.ajax( {timeout : 1000,url : urls,dataType : "jsonp",
						data : {"str" : JSON.stringify(RZCIJSON)},
						contentType : "application/json;utf-8",jsonp : "jsoncallback",
						success : function(x) {
			...
			initControl: function (id) {
				...
				INCJSON.data.sm2xyhexkey = this.settings.AffineX + "|" + this.settings.AffineY;
			...
			intervlOut: function (id, inputID) {
				...
				jsonp({
					...
					callback: "jsoncallback",
					success: function (x) {
						if(parseInt(x.data) == 1){
							var va = pgeCtrl.pwdGetEById(inputID);
							va.blur();
						}
						if (logFlag) console.log(id + "的长度：" + len);
						if (logFlag) console.log("x.data(长度)：" + x.data + ",x.code:" + x.code);
