require(['../../commonConfig'], function () {
    require(['jquery',
        ...
        var table = $("#tradetable").table({
            ...
            {
                field: "tranlog_confirmedvol",
                title: "确认份额(份)",
                render: currencyRender
            },
            {
                field: "tranlog_charge",
                title: "手续费",
                render: currencyRender
            },
            {
        ...
        var syncData = function (params) {
            ...
            httpRequest.post("/" + module_name + "/tradeRecord", requestParams).then(function (sucData) {
        ...
        function applicationamountRender(el, rowData, data) {
            if (rowData.busi_code == "F30001") { //购买必输
        ...
        function busiCodeRender(el, rowData, data) {
            //当baby_code空时，F30001为购买（business_code：020-认购 022-申购），F30002为赎回
            //当baby_code不为空时，F30001为充值，F30002为取现（tranlog_redeemtype：00-快速取现 01-普通取现）
